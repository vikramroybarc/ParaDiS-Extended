...
def load_stress_strain(folder, filename):
    """
    Stress-strain file format used here:
        column 1 = strain
        column 2 = stress

    For stress_Plastic_strain, column 1 is plastic strain.
    For stress_Total_strain, column 1 is total strain.
    """
...
def save_error_plot(x, error, xlabel, ylabel, title, output_file):
    plt.figure(figsize=(8, 5.5))
    plt.plot(x, error, linewidth=1.8)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
    plt.grid(True, alpha=0.3)
...
def main():
    ...
    parser.add_argument(
        "subcycling_folder",
        help=(
            "Folder containing subcycling time_Plastic_strain, density, "
            "stress_Plastic_strain, and stress_Total_strain files."
        ),
    )
    parser.add_argument(
        "trapezoid_folder",
        help=(
            "Folder containing trapezoid time_Plastic_strain, density, "
            "stress_Plastic_strain, and stress_Total_strain files."
        ),
    )
    parser.add_argument(
    ...
    epsp_s, sig_epsp_s = load_stress_strain(sub_folder, "stress_Plastic_strain")
    epsp_t, sig_epsp_t = load_stress_strain(trap_folder, "stress_Plastic_strain")

    epst_s, sig_epst_s = load_stress_strain(sub_folder, "stress_Total_strain")
    epst_t, sig_epst_t = load_stress_strain(trap_folder, "stress_Total_strain")
    ...
    epsp_s, sig_epsp_s = remove_duplicate_times(epsp_s, sig_epsp_s)
    epsp_t, sig_epsp_t = remove_duplicate_times(epsp_t, sig_epsp_t)
    epst_s, sig_epst_s = remove_duplicate_times(epst_s, sig_epst_s)
    epst_t, sig_epst_t = remove_duplicate_times(epst_t, sig_epst_t)
    ...
    save_line_plot(
        ...
        "Plastic strain",
        "Stress",
        "Stress vs plastic strain: subcycling vs trapezoid",
        output / "stress_plastic_strain_comparison.png",
    )
    ...
    save_line_plot(
        ...
        "Total strain",
        "Stress",
        "Stress vs total strain: subcycling vs trapezoid",
        output / "stress_total_strain_comparison.png",
    )
    ...
    plastic_strain_grid = common_grid(epsp_s, epsp_t)

    stress_plastic_sub = np.interp(plastic_strain_grid, epsp_s, sig_epsp_s)
    stress_plastic_trap = np.interp(plastic_strain_grid, epsp_t, sig_epsp_t)

    stress_plastic_error = relative_error(
        stress_plastic_sub,
        stress_plastic_trap,
    )

    total_strain_grid = common_grid(epst_s, epst_t)

    stress_total_sub = np.interp(total_strain_grid, epst_s, sig_epst_s)
    stress_total_trap = np.interp(total_strain_grid, epst_t, sig_epst_t)

    stress_total_error = relative_error(
        stress_total_sub,
        stress_total_trap,
    )
    ...
    save_error_plot(
        plastic_strain_grid,
        stress_plastic_error,
        "Plastic strain",
        "Relative difference (%)",
        "Stress relative difference vs plastic strain",
        output / "stress_plastic_strain_relative_error.png",
    )
    ...
    save_error_plot(
        ...
        "Total strain",
        "Relative difference (%)",
        "Stress relative difference vs total strain",
        output / "stress_total_strain_relative_error.png",
    )
    ...
    stress_plastic_table = np.column_stack(
        (
            plastic_strain_grid,
            stress_plastic_sub,
            stress_plastic_trap,
    ...
    np.savetxt(
        output / "stress_plastic_strain_comparison.csv",
        stress_plastic_table,
        delimiter=",",
        header=(
            "plastic_strain,"
            "subcycling_stress,"
    ...
    stress_total_table = np.column_stack(
        (
            total_strain_grid,
            stress_total_sub,
            stress_total_trap,
    ...
    np.savetxt(
        output / "stress_total_strain_comparison.csv",
        stress_total_table,
        delimiter=",",
        header=(
            "total_strain,"
    ...
    finite_stress_plastic = stress_plastic_error[np.isfinite(stress_plastic_error)]
    if len(finite_stress_plastic):
        print(
            f"Mean stress/plastic difference: "
            f"{np.mean(finite_stress_plastic):.3f} %"
        )
        print(
            f"Max stress/plastic difference : "
            f"{np.max(finite_stress_plastic):.3f} %"
        )

    finite_stress_total = stress_total_error[np.isfinite(stress_total_error)]
    if len(finite_stress_total):
        print(
            f"Mean stress/total difference  : "
            f"{np.mean(finite_stress_total):.3f} %"
        )
        print(
            f"Max stress/total difference   : "
            f"{np.max(finite_stress_total):.3f} %"
        )
    ...
    print("  stress_plastic_strain_comparison.png")
    print("  stress_total_strain_comparison.png")
    print("  stress_plastic_strain_relative_error.png")
    print("  stress_total_strain_relative_error.png")
    print("  plastic_strain_comparison.csv")
    print("  density_comparison.csv")
    print("  stress_plastic_strain_comparison.csv")
    print("  stress_total_strain_comparison.csv")
