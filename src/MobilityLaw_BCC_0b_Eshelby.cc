/**************************************************************************
 *
 *      Module:       Mobility_BCC_0b_Eshelby
...
/**************************************************************************
 *
 *      Function:     Mobility_BCC_0b_Eshelby
 ...
 */
                if ((1.0 - costheta2) > eps) {
                    ...
                    if (isnan(invsqrt1mcostheta2) != 0) {
                        Fatal("Mobility_BCC_0b_Eshelby: invsqrt1mcostheta2 = "
...
#ifdef NAN_CHECK
                    if (isnan(Bglide) != 0) {
                        Fatal("Mobility_BCC_0b_Eshelby: Bglide = NaN\n"
                    ...
                    if (isnan(Bclimb) != 0) {
                        Fatal("Mobility_BCC_0b_Eshelby: Bclimb = NaN\n"
 ...
 */
                if (fabs(burgCryst[X]*burgCryst[Y]*burgCryst[Z]) < eps) {
                   if (nbrs == 2) {
                      ...
                      BsegAlt[0][0] = (dx*dx*BlmBeclE + BeclimbE);
                      BsegAlt[0][1] = (dx*dy*BlmBeclE);
                      BsegAlt[0][2] = (dx*dz*BlmBeclE);
                      BsegAlt[1][1] = (dy*dy*BlmBeclE + BeclimbE);
                      BsegAlt[1][2] = (dy*dz*BlmBeclE);
                      BsegAlt[2][2] = (dz*dz*BlmBeclE + BeclimbE);
                   }
                } else {
/*
 *                Arm is not [0 0 1], so build the drag matrix assuming the
 *                dislocation is screw type
 */
		  BsegAlt[0][0] = (dx*dx * BlmBscE + BscrewE);
		  BsegAlt[0][1] = (dx*dy * BlmBscE);
		  BsegAlt[0][2] = (dx*dz * BlmBscE);
		  BsegAlt[1][1] = (dy*dy * BlmBscE + BscrewE);
		  BsegAlt[1][2] = (dy*dz * BlmBscE);
		  BsegAlt[2][2] = (dz*dz * BlmBscE + BscrewE);
 ...
 */
                if ((1.0 - costheta2) > eps) {
                    ...
                    BsegAlt[0][0] += (nx*nx * BclmBsc +  mx*mx * BglmBsc);
                    BsegAlt[0][1] += (nx*ny * BclmBsc +  mx*my * BglmBsc);
                    BsegAlt[0][2] += (nx*nz * BclmBsc +  mx*mz * BglmBsc);
                    BsegAlt[1][1] += (ny*ny * BclmBsc +  my*my * BglmBsc);
                    BsegAlt[1][2] += (ny*nz * BclmBsc +  my*mz * BglmBsc);
                    BsegAlt[2][2] += (nz*nz * BclmBsc +  mz*mz * BglmBsc);
                }
            }  /* End non-[0 0 1] arm */
