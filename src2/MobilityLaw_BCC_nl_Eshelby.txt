/**************************************************************************
 *
 *      Module:       MobilityLaw_BCC_nl_Eshelby.cc
 *      Description:  Hybrid mobility for nodes in BCC metals.  Arm
 *                    portions outside Eshelby inclusions use BCC_nl;
 *                    portions inside use the BCC_0 drag tensor.
 *
...
/*
 *      Return the fraction of the nodal drag integral associated with the
 *      union of this arm's inclusion-covered intervals.  The normalized arm
 *      coordinate s starts at <node>, so its nodal shape function is 1-s.
 */
static real8 EshelbyNodalFraction(Home_t *home, Node_t *node,
                                  Node_t *nbrNode, real8 rt[3])
{
        int                 i, j, numLimits;
        real8               factor, limits[NUMLIMITS_MAX][2];
        real8               pos1[3], pos2[3];
        Param_t            *param;
        SegPartIntersect_t *intersection;

        if (!home->param->enableInclusions) {
            return(0.0);
        }
        ...
        for (i = 0; i < intersection->numIntersections; i++) {
            int           isIntercept, ninc;
            real8         dx, dy, dz, partPos[3], ratio[2];
            EInclusion_t *inclusion;

            inclusion = &home->eshelbyInclusions[
                            intersection->inclusionIndex[i]];
            ...
            isIntercept = IntersectionEllipseSegment(partPos,
                                  inclusion->radius, inclusion->rotation,
                                  pos1, pos2, 1.0, &ninc, ratio);
            ...
            if (numLimits >= NUMLIMITS_MAX) {
                Fatal("EshelbyNodalFraction: number of inclusion intervals "
                      "(%d) exceeds maximum (%d)",
                      numLimits + 1, NUMLIMITS_MAX);
            }

            limits[numLimits][0] = ratio[0];
            limits[numLimits][1] = ratio[1];
            numLimits++;
        }
...
/*
 *      Sort by lower endpoint, then merge overlapping/touching intervals.
 */
        for (i = 1; i < numLimits; i++) {
            real8 lo = limits[i][0];
            real8 hi = limits[i][1];

            for (j = i; (j > 0) && (limits[j-1][0] > lo); j--) {
                limits[j][0] = limits[j-1][0];
                limits[j][1] = limits[j-1][1];
            }
        ...
        for (i = 1; i < numLimits; i++) {
            if (limits[i][0] <= hi) {
                hi = MAX(hi, limits[i][1]);
            } else {
                factor += (1.0-lo)*(1.0-lo) - (1.0-hi)*(1.0-hi);
                lo = limits[i][0];
                hi = limits[i][1];
            }
        }
...
/*
 *      Build the per-unit-length drag tensor used by Mobility_BCC_0().
 *      <burg> and <lineDir> are in the crystal frame, matching the frame in
 *      which the copied BCC_nl Newton solve operates.
 */
static void BuildBCC0DragMatrix(Param_t *param, real8 burg[3],
                                real8 lineDir[3], real8 drag[3][3])
{
        int   m, n;
        real8 bMag2, costheta, costheta2, invsqrt1mcostheta2;
        real8 Bscrew, Bedge, Beclimb, Bline, Bglide, Bclimb;
        ...
        bMag2 = DotProduct(burg, burg);
        costheta = DotProduct(lineDir, burg);
        costheta2 = costheta * costheta / bMag2;
        ...
        if (fabs(burg[X]*burg[Y]*burg[Z]) < eps) {
            for (m = 0; m < 3; m++) {
                for (n = 0; n < 3; n++) {
                    drag[m][n] = Beclimb * (m == n) +
                                 (Bline-Beclimb) * lineDir[m] * lineDir[n];
                }
            }
            return;
        }
...
/*
 *      Start with the pure-screw tensor.
 */
        for (m = 0; m < 3; m++) {
            for (n = 0; n < 3; n++) {
                drag[m][n] = Bscrew * (m == n) +
                             (Bline-Bscrew) * lineDir[m] * lineDir[n];
            }
        }

/*
 *      Apply the same mixed-character interpolation as Mobility_BCC_0().
 */
        if ((1.0-costheta2) > eps) {
            invsqrt1mcostheta2 = 1.0 /
                                 sqrt((1.0-costheta2) * bMag2);

            cross(burg, lineDir, nDir);
            nDir[X] *= invsqrt1mcostheta2;
            nDir[Y] *= invsqrt1mcostheta2;
            nDir[Z] *= invsqrt1mcostheta2;
            cross(nDir, lineDir, mDir);

            invBscrew2 = 1.0 / (Bscrew*Bscrew);
            invBedge2  = 1.0 / (Bedge*Bedge);

            Bglide = 1.0 / sqrt(invBedge2 +
                                (invBscrew2-invBedge2) * costheta2);
            Bclimb = sqrt(Beclimb*Beclimb +
                          (Bscrew*Bscrew-Beclimb*Beclimb) * costheta2);

            for (m = 0; m < 3; m++) {
                for (n = 0; n < 3; n++) {
                    drag[m][n] += (Bclimb-Bscrew) * nDir[m] * nDir[n] +
                                  (Bglide-Bscrew) * mDir[m] * mDir[n];
                }
            }
        }
...
/**************************************************************************
 *
 *      Function:     Mobility_BCC_nl_Eshelby
...
int Mobility_BCC_nl_Eshelby(Home_t *home, Node_t *node, MobArgs_t *mobArgs)
{
        ...
        real8   vtest[3], fn[3], rt[3], rtLab[3], zero3[3], tmp3[3], ferror[3];
 ...
 */
        if (numNbrs > MAX_NBRS) {
            Fatal("Mobility_BCC_nl_Eshelby: Node segment count (%d) exceeds\n"
 ...
 */
        for (i = 0, iAll = 0; iAll < numNbrs; i++, iAll++) {
            ...
            VECTOR_COPY(rtLab, rt);

/*
 *          Follow the original BCC_nl ordering for all crystal-frame
 *          geometry.  The saved lab-frame copy is used only to intersect
 *          the arm with Eshelby inclusions.
 */
            if (param->useLabFrame) {
                real8 lineRot[3];
 ...
 *          skip the segment.  Check before evaluating inclusion geometry,
 *          which requires a nonzero arm.
 */
...
/*
 *          Compute the inclusion-covered nodal fraction while the arm is
 *          still in the simulation frame used by the inclusion geometry.
 */
            insideFraction[i] = EshelbyNodalFraction(home, node, nbrNode,
                                                      rtLab);

            if (insideFraction[i] > 0.0) {
                hasInsideBCC0 = 1;
            }
...
/*
 *          The inclusion-covered portion owns a full BCC_0 arm tensor;
 *          it is not decomposed into BCC_nl screw and edge lengths.
 */
            if (insideFraction[i] > 0.0) {
                real8 bcc0Drag[3][3];
                real8 insideLength = 0.5 * LTotal[i] * insideFraction[i];

                BuildBCC0DragMatrix(param, burg[i], linedir[i], bcc0Drag);

                for (m = 0; m < 3; m++) {
                    for (n = 0; n < 3; n++) {
                        dflindv[m][n] += insideLength * bcc0Drag[m][n];
                    }
                }
            }
 ...
 */
            ...
            if (fabs(burgProd) < eps) {
                ...
                for (m = 0; m < 3; m++) {
                    for (n = 0; n < 3; n++) {
                        dflindv[m][n] += (0.5 * LTotal[i] *
                                         (1.0-insideFraction[i])) *
                                         dfjdragdv[m][n];
                    }
                }
 ...
 */
        ...
        for (i = 0, iAll = 0; iAll < numNbrs; i++, iAll++) {
            ...
            if (fabs(burgProd) > eps) {
                if (screwExists[i] && (insideFraction[i] < 1.0)) {
                    thisScrew = -1;
 ...
 */
        if ((numscrews == 1) && (isJunction == 0) && (!hasInsideBCC0)) {
            for (i = 0; i < 3; i++) {
